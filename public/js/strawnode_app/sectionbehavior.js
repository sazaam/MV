...
module.exports = {
    ...
    ensureTranslates:function(res, project_zone){
        ...
        if(translates.size()){
            translates.each(function(i, el){
                ...
                if(ttt.test('^jade::')){
                    var content = jade.render(ttt.replace('jade::', ''), {filename:'/jade/index.jade'}) ;
                    var cont = $(marked.marked(content)).html() ;
                    hasJade = 1 ;
                }
                ...
                setTimeout(() => {
                    tt.textAppear(desc, hasJade ? cont : newtrans, unfound) ;
                }, 2);
    ...
    deep_slides:function deep_slides(cond, res){
        ...
        if(cond){
            ...
            if(!noSlide){
                ...
                project_zone.on(events.down, res.userData.onPJ_MD = function(e){
                    ...
                    if(e.currentTarget == e.target) {
                        ...
                        window.location.hash = $('.close_project a').attr('href') ;
			...
			project_zone.off('mousedown', res.userData.onPJ_MD) ;
