...
(function(name, definition){
	...
	return Pkg.write('org.libspark.straw', function(){
		...
		var Kompat = Type.define({
			...
			statics : {
				...
				initialize:function(){
					...
					kompat.ES6BaseCompliant = ctor.checkES6BaseCompliant() ;
					...
					if(brave) {
						name = kompat.name = 'Brave' ;
						version = kompat.version = brave.toString() ;
					}
		...
		var Global = Type.define({
			...
			statics:{
				...
				fire:function(tg, e){
					...
					Global.loop(handlers, function fireHandlers(h, j, handlers){
						if(!!!h) return ;
		...
		var Response = Type.define({
			...
			constructor:Response = function Response(id, pattern, commandOpen, commandClose){
				...
				Response.base.apply(this, [
					...
					commandOpen || new Command(res, function resCommandOpen(){
						...
						var c = this ;
						// dirty way to remove double-bindings of focusOut event, but no workaround found yet...
						try {
						...
						if(!!res.responseAct) {
							...
							if(!!rr){
								try {
									...
									// res.unbind('focusOut', focus) ; // is handled in the next opening
								}
							}
						}
						res.unbind('focusIn', focus) ;
				...
				if(!!callback){ // switch to ASYNC MODE
					...
					try{
						request = new Request().load(true, url + hash, function(jxhr, r){
							t = request.response ;
							res.template = res.packResponse(t, url, params) ;
					...
					try{
						request = new Request().load(false, url /* + hash */, null, null, null, true) ;
						t = request.response ;
					}catch(e){
						e.message = ('LoadModuleError :: URL ' + url + 'not found') ;
						throw e ;
			...
			packResponse:function(t, url, params){
				// So this will work with dyn sections
				// TODO : fix all this in case we want to implement 404 in dynsections
				...
				var s = /.jade$/.test(url) ? new Jade().render(t, params) : t ;
			...
			send:function(t, url, params){
				...
				params.filename = url ;
				var s = new Jade().render(t, params) ;
